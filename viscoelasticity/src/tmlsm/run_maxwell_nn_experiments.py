"""
Maxwell NN Experiment Training Script
=====================================
Trains Maxwell NN models (learned evolution equation) across multiple configurations.

Same experiment groups as GSM and RNN sweeps:
...
Usage:
    cd viscoelasticity
    python -m tmlsm.run_maxwell_nn_experiments
...
# =============================================================================
# Experiment Configurations (identical to GSM and RNN sweeps)
# =============================================================================
...
def save_model(model, history_losses, config_name, seed, steps, n_ts, artifacts_dir):
    ...
    model_filename = f"maxwell_nn__{config_name}__seed_{seed}__{steps}steps__{n_ts}ts__{timestamp}.eqx"
...
def run_all_experiments():
    """Run all Maxwell NN experiments."""

    artifacts_dir = Path("artifacts") / "maxwell_nn_experiments"
    ...
    print("=" * 70)
    print("MAXWELL NN EXPERIMENT TRAINING")
    print("=" * 70)
    ...
    for config_name, config in EXPERIMENTS.items():
        ...
        for seed_idx in range(N_SEEDS):
            ...
            # Build fresh Maxwell NN model
            model = tm.build_maxwell_nn(key=key_init)
